...
public class ProjectionService {
    ...
    public List<Task> getProjection(Long stationId) {
        ...
        if (optionalStation.isPresent()) {
            if(optionalStation.get().getProjection() != null) {
                return optionalStation.get().getProjection();
            }
        }
        return null;
    }
    ...
    public Task updateProjection(Task task) {
        ...
        if(optionalTask.isEmpty()){
            ...
            if(saveTask.getDone() != task.getDone()){
                if(task.getDone()){
                    historyText = "Projektierung '" + saveTask.getTaskSetting().getName() + "' auf 'erledigt' geändert";
                } else {
                    historyText = "Projektierung '" + saveTask.getTaskSetting().getName() + "' auf 'nicht erledigt' geändert";
                }
            } else if(saveTask.getCommited() != task.getCommited()){
                if(task.getCommited()){
                    historyText = "Projektierung '" + saveTask.getTaskSetting().getName() + "' auf 'übergeben' geändert";
                } else{
                    historyText = "Projektierung '" + saveTask.getTaskSetting().getName() + "' auf 'nicht übergeben' geändert";
                }
        ...
        if(optionalProject.isPresent()) {
            ...
            for(Station station : stationList) {
                if (stationRepository.findStationByNameAndProjectionTaskSettingId(station.getName(), taskSetting.getId()).isEmpty()) {
                    ...
                    log.info("Added Projection Task '" + task.getTaskSetting().getName() + "' to station '" + station.getName() + "'");
                }
            }
        }
    }
