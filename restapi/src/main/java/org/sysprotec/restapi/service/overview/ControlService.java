...
public class ControlService {
    ...
    public List<Task> getControl(Long stationId) {
        ...
        if (optionalStation.isPresent()) {
            if(optionalStation.get().getControl() != null) {
                return optionalStation.get().getControl();
            }
        }
        return null;
    }
    ...
    public Task updateControl(Task task) {
        ...
        if(optionalTask.isEmpty()){
            ...
            if(saveTask.getDone() != task.getDone()){
                if(task.getDone()){
                    historyText = "Kontrolle '" + saveTask.getTaskSetting().getName() + "' auf 'erledigt' geändert";
                } else {
                    historyText = "Kontrolle '" + saveTask.getTaskSetting().getName() + "' auf 'nicht erledigt' geändert";
                }
            } else if(saveTask.getCommited() != task.getCommited()){
                if(task.getCommited()){
                    historyText = "Kontrolle '" + saveTask.getTaskSetting().getName() + "' auf 'übergeben' geändert";
                } else{
                    historyText = "Kontrolle '" + saveTask.getTaskSetting().getName() + "' auf 'nicht übergeben' geändert";
                }
        ...
        if(optionalProject.isPresent()) {
            ...
            for(Station station : stationList) {
                if (stationRepository.findStationByNameAndControlTaskSettingId(station.getName(), taskSetting.getId()).isEmpty()) {
                    ...
                    log.info("Added Control Task '" + task.getTaskSetting().getName() + "' to station '" + station.getName() + "'");
                }
            }
        }
    }
