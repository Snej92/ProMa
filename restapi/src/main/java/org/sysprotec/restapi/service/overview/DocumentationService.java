...
public class DocumentationService {
    ...
    public List<Task> getDocumentation(Long stationId) {
        ...
        if (optionalStation.isPresent()) {
            if(optionalStation.get().getDocumentation() != null) {
                return optionalStation.get().getDocumentation();
            }
        }
        return null;
    }
    ...
    public Task updateDocumentation(Task task) {
        ...
        if(optionalTask.isEmpty()){
            ...
            if(saveTask.getDone() != task.getDone()){
                if(task.getDone()){
                    historyText = "Doku '" + saveTask.getTaskSetting().getName() + "' auf 'erledigt' geändert";
                } else {
                    historyText = "Doku '" + saveTask.getTaskSetting().getName() + "' auf 'nicht erledigt' geändert";
                }
            } else if(saveTask.getCommited() != task.getCommited()){
                if(task.getCommited()){
                    historyText = "Doku '" + saveTask.getTaskSetting().getName() + "' auf 'übergeben' geändert";
                } else{
                    historyText = "Doku '" + saveTask.getTaskSetting().getName() + "' auf 'nicht übergeben' geändert";
                }
            ...
            if(optionalProject.isPresent()) {
                ...
                for(Station station : stationList) {
                    if (stationRepository.findStationByNameAndDocumentationTaskSettingId(station.getName(), taskSetting.getId()).isEmpty()) {
                        ...
                        log.info("Added Documentation Task '" + task.getTaskSetting().getName() + "' to station '" + station.getName() + "'");
                    }
                }
            }
        }
